...
prefixvarinput = input("Choose a command symbol: 1 = /, 2 = ?, 3 = ! ")
if prefixvarinput == "1":
    prefixvar = "/"
...
jokelist = ["There are currently no jokes right now. Use the joke report command to add some."]
coins = ["It landed on heads!", "It landed on tails!"]
client = discord.Client()
bot = commands.Bot(command_prefix=prefixvar, description="This is a Helper Bot, that fits almost any discord server. Made by DINO/PING BIT.", help_command=None, activity = activity, status=discord.Status.online)
...
@bot.command()
async def info(ctx):
    embed = discord.Embed(title=f"{ctx.guild.name}", description="Server information", timestamp=datetime.datetime.utcnow(), color=discord.Color.blue())
...
async def kick(ctx, member: discord.Member, *, reason=None):
    if ctx.message.author.guild_permissions.administrator:
        ...
        await ctx.message.channel.send("You do not have Admin.")
        print("Member attempted to kick. Did not have admin, or bot's role was obstructed. Report from: " + ctx.guild.name)
...
async def ban(ctx, member: discord.Member, *, reason=None):
    if ctx.message.author.guild_permissions.administrator:
        ...
        await ctx.message.channel.send("You do not have Admin.")
        print("Member attempted to ban. Did not have admin, or bot's role was obstructed. Report from: " + ctx.guild.name)
...
async def changenick(ctx, member: discord.Member, *, nick):
    if ctx.message.author.guild_permissions.administrator:
        ...
        print("Nickname was changed for user: " + str(member) + " Their new nickname is: " + nick + ". Report from: " + ctx.guild.name)
    else:
        await ctx.message.channel.send("You do not have the permissions to change usernames.")
        print("Member attempted to change a nickname. Did not have admin, or bot's role was obstructed. Report from: " + ctx.guild.name)
...
async def jokerep(ctx, *,jrep):
    await ctx.message.channel.send("Your joke was recorded. It will be added if the developer aporoves it. (Could be a few hours.)")
...
async def setdelay(ctx, seconds: int):
    if ctx.message.author.guild_permissions.administrator:
        ...
        print("Member attempted to set slowmode. Did not have admin, or bot's role was obstructed. Report from: " + ctx.guild.name)
...
@bot.command()
async def help(ctx):
    embed=discord.Embed(title="Command Help", description="Help for all current commands this discord bot supports.", color=0xf084c8)
    ...
    embed.add_field(name=f"{prefixvar}changenick [member] [new name]", value="changes a user's nick-name.", inline=False)
    embed.add_field(name=f"{prefixvar}coinflip", value="Randomly picks between heads, and tails.", inline=False)
    embed.add_field(name=f"{prefixvar}joke", value="Randomly picks a joke from my joke database. You can add a joke using the jokeadd command.", inline=False)
    embed.add_field(name=f"{prefixvar}jokerep [joke]", value="Adds a joke to the bot's database, to be used for the joke command.", inline=False)
    embed.add_field(name=f"{prefixvar}membercount", value="Tells you the amount of members in a server.", inline=False)
    embed.add_field(name=f"{prefixvar}servercount", value="Tells you the amount of servers the bot is in.", inline=False)
    embed.add_field(name=f"{prefixvar}report [error/suggestion]", value="Allows you to suggest a feature, or report a bug.", inline=False)
    embed.add_field(name=f"{prefixvar}setdelay [time in seconds]", value="Changes the current channel's slowmode. (Setting it to 0 removes slowmode)")
    embed.add_field(name=f"{prefixvar}wikisearch [Search term]", value="Searchs Wikipedia for information, and displays it.", inline=False)
    embed.set_footer(text=f"Command missing? message PING BIT#9644 for help or go to bot help. Made by Dino/PING BIT. (Command prefix: {prefixvar})")
